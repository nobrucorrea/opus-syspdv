...
public abstract class Retaguarda extends Componente {
    ...
    public Retaguarda() {
        ...
        produtoID = ConfiguracaoContexto.getInstance().getConfiguracao().getProperty("banco.produto.id");
    ...
    public void atualizar(List<Produto> produtos) {
        ...
        //concatenacao para resolver problema de mudanca no capo de identificao 
        //no campo de produtos
        StringBuilder query = new StringBuilder();
        query.append("SELECT ");
        query.append(produtoID);
        query.append(" FROM produto");           
        logger.info("%s", query.toString());
        List<String> produtosDB = (List<String>) executeQuery(query.toString());
        ...
        for (Produto produto : produtos) {
            String codigoProduto = OpusUtils.completeToLeft(produto.getCodigoInterno(), '0', 14);
            //logger.info("[%s] é gual a {%s} ", produto.getCodigoInterno(), codigoProduto);
